...
protocol HomeFeedsCellDelegate: class {
...
class HomeFeedsCell: UITableViewCell {

    /// 头像
    @IBOutlet weak var iconImage: UIImageView!{
        didSet{
            iconImage.layer.cornerRadius = 400
        }
    }
    /// 姓名
    ...
    weak var delegate: HomeFeedsCellDelegate?
    var indexPath: NSIndexPath = NSIndexPath(forRow: 0, inSection: 0)
    
    @IBAction func shareButtonDidPressed(sender: UIButton) {
        delegate?.HomeFeedsCellShare(indexPath)
    }
    
    @IBAction func starButtonDidPressed(sender: UIButton) {
        delegate?.HomeFeedsCellStar(indexPath)
    }
    
    @IBAction func commentButtonDidPressed(sender: UIButton) {
        
        delegate?.HomeFeedsCellComment(indexPath)
    }
    ...
    var status: Feed? {
        didSet {
            ...
            timeLabel.text = status!.postTime
            ...
            // 头像
            if let iconUrl = status?.avatarImageUrl {
...
//            }
    ...
    ///  返回可重用标识符
    class func cellIdentifier(status: Feed) -> String {
        return "HomeCell"
    }
    
    /// 定义一个图片被选择的闭包
    var photoDidSelected:((status:Feed,photoIndex:Int) ->())?
...
extension HomeFeedsCell:UICollectionViewDataSource,UICollectionViewDelegate{
    ...
    func collectionView(collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        // 配图的数量
        return status?.postImageUrls.count ?? 0
    }
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        cell.urlString = status!.postImageUrls[indexPath.row]
    ...
    */
    func calsPictureViewSize() -> (itemSize:CGSize,viewSize:CGSize){
        ...
        let count = status?.postImageUrls.count ?? 0
        ...
        if count == 1{
            let path = NetworkManager.sharedManager.fullImageCachePath(status!.postImageUrls[0])
