...
{
    [self.delegate UUInputFunctionViewMethod:self sendVoice:voiceData time:playTime+1];
...
- (void)sendMessage:(UIButton *)sender
{
    if (self.isAbleToSendTextMessage) {
        ...
        [self.delegate UUInputFunctionViewMethod:self sendMessage:resultStr];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [self.superVC dismissViewControllerAnimated:YES completion:^{
        [self.delegate UUInputFunctionViewMethod:self sendPicture:editImage];
    }];
}
